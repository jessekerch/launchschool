...
function thirdMax(array) {
  ...
  while (numbers.length > 0 && highNums.length < 3) {
    let highNum = Math.max(...numbers);
    highNums.push(highNum);
    let highNumIdx = numbers.indexOf(highNum);
    numbers.splice(highNumIdx, 1);
  }
