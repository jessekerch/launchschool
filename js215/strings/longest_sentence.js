...
function getLongest(sentences) {
  let longestSentence = '';
  let longestLength = 0;
  sentences.forEach((sentence) => {
    currentLength = getWordCount(sentence);
  ...
  return [longestSentence, longestLength];
}
...
function invalidText(text) {
  if (text.match(/[_A-Za-z0-9][.!?]/g) === null) {
    console.log('No sentences found in text!');
    return true;
  };
...
function longestSentence(text) {
  ...
  let [longestSentence, longestLength] = getLongest(sentences);
